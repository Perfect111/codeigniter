...
class Db{
	...
	public function select_templates($userId) {
		try {
			$sql='SELECT * FROM `bal_email_builder` WHERE `UserId`='.$userId;
			$conn=$this->connect();
			$result = mysqli_query($conn, $sql);
			$newArr=array();
	...
	public function deleteTemplate($templateId,$UserId) {
		...
		try {
			$conn=$this->connect();
			$stmt = $conn->prepare("DELETE FROM `bal_email_builder`  WHERE ID=? AND UserId=?");
	...
	public function update($name,$content,$id) {
		...
		try {
			$conn=$this->connect();
			$stmt = $conn->prepare("UPDATE `bal_email_builder` SET `name`=?,`content`=? WHERE id=?");
			$stmt->bind_param("sss", $name, $content,$id);
