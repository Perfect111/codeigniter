...
class Dashboard_main_helper {
    ...
    /*
     * initial settings variables loads
     */
    static function init_variables() {
        ...
        $variables = $CI->db->get_where ( $_table_prefix . 'variables', array (
                'account_id' => 0 
        ) )->result_array ();
        if ($account_id) {
            $variables = array_merge ( $variables, $CI->db->get_where ( $_table_prefix . 'variables', array (
                    'account_id' => $account_id 
            ) )->result_array () );
        }
        if (isset ( $variables ) && is_array ( $variables ) && ! empty ( $variables )) {
            foreach ( $variables as $variable ) {
                $CI->config->set_item ( $variable ['key'], $variable ['value'] );
            }
        }
    }
}
...
/*
 * this function renders left menu
 */
function boo2_render_left_menu($base_url = '') {
    ...
    foreach ( $menu_items as $menu ) {
        ...
        $langkey_menu = '_' . strtoupper ( $menu );
        $output_html .= "<a class='menu-anchor {$selecte_item}' href='" . $base_url . $controller_sub_folder . '/' . $menu . "/{$listing_view}'>" . dashboard_lang ( $langkey_menu ) . "</a>";
        $output_html .= "</li>";
    }
    echo $output_html;
}
 ...
 */
function boo2_render_form_label($text, $description, $required='') {
    $position = "top";
    $text = dashboard_lang("_".strtoupper($text));
    $html = "<label class='pop_over control-label' name='" . $text . "' data-toggle='tooltip' data-placement='" . $position . "' title=''>";
    $html .= $text;
    if((bool)$required == 1){
...
if ( ! function_exists('form_dropdown_auto'))
{
     ...
     * @return	string
     */
    function form_dropdown_auto( $name="", $selected = '', $options= array(), $ref_table = '', $key = '', $value = '', $extra= '', $field_status='' )
    {
        ...
        if(isset($selected) && !empty($selected)){
            ...
            $html = '<select ref-table="'.$ref_table.'" ref-key="'.$key.'" ref-value="'.$value.'" name="'.$name.'" id="'.$name.'" class="'.$name.' '.$extra.' form-control" '.$field_status.'>
 ...
 */
function boo2_render_image($src_image, $img_upload_path, $sub_directory, $name, $field_status) {
    $html = '<div>';
    if ((stripos ( $src_image, "http://" ) !== false) || (stripos ( $src_image, "https://" ) !== false)) {
        ...
        if(!empty($src_image )){
            ...
            $html .= '<a class="fancybox" href="' . $image_path . '"> <img width="100" src="' . $image_path . '"> </a>';
            $html .= '<input type="checkbox" name="'.$name.'" value="1" '.$field_status.'/>' . dashboard_lang ( '_DELETE_THIS_IMAGE' );
        }
 ...
 * renders image in a form
 */
function boo2_render_file($src_file, $file_upload_path, $name, $field_status) {
    
    $html = '<div>';
    if ((stripos ( $src_file, "http://" ) !== false) || (stripos ( $src_file, "https://" ) !== false)) {
        $html .= '<a href="' . $src_file . '">' . $src_file . '</a>';
    } else {
        if(!empty($src_file)) {
            ...
            $html .= '</span>';
            $html .= '<input type="checkbox" name="' . $name . '" value="1" ' . $field_status . '/>' . dashboard_lang('_DELETE_THIS_FILE');
        }
    }
...
function get_user_opening_view($user_role){
    ...
    $data = $CI->db->get_where("{$user_roles_table}", array("slug" => "$user_role"))->row();
...
function boo2_render_top($additional_buttons = array()) {
    ...
    return $CI->load->view ( 'core_' . $CI->config->item ( "template_name" ) . '/helper_render_action', $data );
}
...
function boo2_render_edit_top() {
    ...
    $data ['edit_path'] = $dashboard_helper->get ( 'edit_path' );
    return $CI->load->view ( 'core_' . $CI->config->item ( "template_name" ) . '/helper_render_edit_button', $data );
}
/*
...
function format_price ($money_format, $price) {
    ...
    if ($check_decimal_exists !== false ){
        ...
        }else if (strlen($after_decimal_price) == 1){
...
function getAllMenu(){
    ...
    $CI->db->where('account_id', get_account_id());
...
function getAllPermittedRoles($account_id){
    ...
    if(get_user_role()!= 'super_admin'){
        $CI->db->where('user_create_by', get_user_role());
    }
...
function get_auto_ins_number($tbl_name, $name){
    
    $CI = get_instance();    
    $start_number = 1;
    $max_num = 0;
    $setting_start_number = $CI->config->item('#AUTO_INCREMENT_NUMBER_START_FROM');
